...
Listener code is under `lib/events/`. It has 3 `.js` files -- `encoding.js`, `handlers.js`, `subscriber.js` -- roles of each of them is reflected in name, and a `proto.json` file, used only in `encoding.js` in encoding/decoding of events and subscription requests with their proto structures.
